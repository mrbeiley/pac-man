...
class PriorityQueue:
    ...
    def find_and_extract(self, state):
        """
        Finds index of node in self.heap list that matches state
        If found, removes element from self.heap list and returns element
        If no match found, returns None (which may indicate a problem you
        expected there to be a match, e.g., due to repreated state visit.
        NOTE: assumes element accessor containing state is named 'state'.
        """
        for i, (_, _, item) in enumerate(self.heap):
            if item.state == state:
                del self.heap[i]  # remove the item from the heap (as list)
